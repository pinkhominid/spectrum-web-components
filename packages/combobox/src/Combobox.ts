...
export class Combobox extends SpectrumElement {
    ...
    @property()
    public autocomplete: 'list' | 'none' = 'none';
    ...
    // public onListPointerenter(event: PointerEvent): void {
    //     const descendent = event.composedPath().find(
    //         el => typeof (el as MenuItem).value !== 'undefined'
    //     );
        ...
        entries.forEach((entry) => {
            if ((entry.target as HTMLElement).children.length) {
                this.positionListbox(
                    (entry.target as HTMLElement).children[0] as HTMLElement
                );
            }
        });
    }
    ...
    protected render(): TemplateResult {
        ...
        return html`
            ...
            <sp-textfield
                ...
                @blur=${({ target }: Event) =>
                    ((target as Textfield).focused = true)}
            ></sp-textfield>
            <sp-action-button
                aria-controls="listbox"
                aria-expanded=${this.open ? 'true' : 'false'}
            ...
            <sp-popover
                ...
                ?hidden=${!this.open ||
                (this.open && this.availableOptions.length < 2)}
                role="listbox"
                id="overlay"
                style="max-height: 100%;"
            >
                <sp-menu style="width: ${width}px;" selects="single">
            ...
            <sp-popover
                aria-labelledby="label"
                tabindex="-1"
                ?open=${this.open && this.availableOptions.length > 1}
                role="listbox"
                id="listbox"
                style="overflow: auto;"
            >
    ...
    protected updated(changed: PropertyValues<this>): void {
        ...
        if (changed.has('value')) {
            if (this.overlay) this.positionListbox(this.overlay);
        }
    }
